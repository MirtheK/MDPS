...
class CheckpointSequential(TimestepEmbedSequential):
    def forward(self, x, emb):
        # We only apply checkpointing to the attention blocks
        for layer in self:
            if isinstance(layer, AttentionBlock):
                x = checkpoint.checkpoint(layer, x, emb, use_reentrant=False)
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.down = nn.ModuleList(
                [CheckpointSequential(nn.Conv3d(self.in_channels, base_channels, 3, padding=1))]
                )
        channels = [ch]
        ds = 1
        for i, mult in enumerate(channel_mults):
            for _ in range(num_res_blocks):
                ...
                self.down.append(CheckpointSequential(*layers))
                channels.append(ch)
        ...
        self.middle = CheckpointSequential(
                ResBlock(
                        ch,
                        time_embed_dim=time_embed_dim,
                        dropout=dropout
        ...
        for i, mult in reversed(list(enumerate(channel_mults))):
            for j in range(num_res_blocks + 1):
                ...
                if i and j == num_res_blocks:
                    ...
                    ds //= 2
                self.up.append(CheckpointSequential(*layers))

        self.out = nn.Sequential(
                GroupNorm32(32, ch),
                nn.SiLU(),
                zero_module(nn.Conv3d(base_channels * channel_mults[0], self.out_channels, 3, padding=1))
                )
