...
#         # Use the pre-computed et (detached), avoid a second UNet forward pass
#         et_detached = et.detach()
#         x_0_hat = (x_in - et_detached * (1 - at).sqrt()) / at.sqrt()
    ...
    with torch.no_grad():
        ...
        for index in range(len(seq_list)):
            ...
            if w == 0:
                ...
                guidance = condition_score(xt, et, y0, t, at, config)
                et_hat = et + (1 - at).sqrt() * w * guidance
            ...
            # xt is overwritten each step — no list, no accumulation
