    ...
    for epoch in range(n_epochs):
        ...
        for i, batch in enumerate(train_loader):  
            ...
            with autocast('cuda', enabled=True):
                ...
                # we only sample the 0 to t_lambda (< T) time steps
    ...
    for i, batch in enumerate(test_loader):
        ...
        with autocast('cuda', enabled=True):
            with torch.no_grad():
                for t in timesteps:
                    # Get model prediction
                    model_output = model(x_t, timesteps=torch.Tensor((t,)).to(image_test.device), context=None)
